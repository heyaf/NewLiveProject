...
- (UIView *)searchHistoryView
{
    if (!_searchHistoryView) {
        if (_historyArray.count > 0) {
            self.searchHistoryView = [self setViewWithOriginY:0 title:@"历史记录" textArr:self.historyArray];
        } else {
...
- (UIView *)setViewWithOriginY:(CGFloat)riginY title:(NSString *)title textArr:(NSMutableArray *)textArr
{
    ...
    UILabel *titleL = [[UILabel alloc] initWithFrame:CGRectMake(15, 10, KScreenW - 30 - 45, 30)];
    titleL.text = title;
    ...
    if ([title isEqualToString:@"历史记录"]) {
        ...
        btn.frame = CGRectMake(KScreenW - 45, 10, 28, 30);
    ...
    for (int i = 0; i < textArr.count; i++) {
        ...
        if (letfWidth + width + 15 > KScreenW) {
...
//        }
    ...
    view.frame = CGRectMake(0, riginY, KScreenW, y + 40);
    return view;
}
...
{
    UIView *historyView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, KScreenW, 80)];
    UILabel *titleL = [[UILabel alloc] initWithFrame:CGRectMake(15, 10, KScreenW - 30, 30)];
    titleL.text = @"历史记录";
    titleL.font = [UIFont systemFontOfSize:15];
    titleL.textColor = [UIColor blackColor];
    titleL.textAlignment = NSTextAlignmentLeft;
...
{
    CGFloat width = [text boundingRectWithSize:CGSizeMake(KScreenW, 40) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:12]} context:nil].size.width;
    return width;
}
