...
- (void)playOrPauseAction{
    if (!isPlay) {
        [self.xjPlayer play];
        isPlay = YES;
        [self.playOrPauseBtn setImage:[UIImage imageNamed:@"live_icon_pause"] forState:UIControlStateNormal];
        ...
        [self.xjPlayer pause];
        isPlay = NO;
        [self.playOrPauseBtn setImage:[UIImage imageNamed:@"live_icon_play"] forState:UIControlStateNormal];
        if ([self.delegate respondsToSelector:@selector(xjPlayerPlayOrPause:)]) {
...
- (void)fullOrSmallAction{
    if (isFull) {
        ...
        [self.fullOrSmallBtn setImage:[UIImage imageNamed:@"icon_fullscreen"] forState:UIControlStateNormal];
        isFull = NO;
        ...
        [self.fullOrSmallBtn setImage:[UIImage imageNamed:@"icon_smallscreen"] forState:UIControlStateNormal];
        isFull = YES;
        if ([self.delegate respondsToSelector:@selector(xjPlayerFullOrSmall:)]) {
...
- (void)playSliderValueChanging:(id)sender{
    ...
    if (slider.value == 0.0000) {
        [self.xjPlayer seekToTime:kCMTimeZero completionHandler:^(BOOL finished) {
            ...
            [weakSelf.playOrPauseBtn setImage:[UIImage imageNamed:@"live_icon_pause"] forState:UIControlStateNormal];
            isPlay = YES;
        }];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:@"status"]) {
        if (playerItem.status == AVPlayerItemStatusReadyToPlay) {
            ...
            [MBProgressHUD showError:@"播放未知，请稍后重试"];
            UIViewController *vc = [self currentVc];
            [vc.navigationController popViewControllerAnimated:YES];
        }else if (playerItem.status == AVPlayerStatusFailed){
            [MBProgressHUD showError:@"播放失败，请稍后重试"];
            UIViewController *vc = [self currentVc];
            [vc.navigationController popViewControllerAnimated:YES];
        }
...
#pragma mark   -------------------------------视屏播放完后的通知事件。从头开始播放；
...
- (void)setXjPlayerUrl:(NSString *)xjPlayerUrl{
    ...
    if (!isFirst) {
        ...
        [Bbtn setBackgroundImage:IMAGE_NAMED(@"back") forState:UIControlStateNormal];
        Bbtn.frame = CGRectMake(20, 20, 30, 30);
        UIViewController *vc = [self currentVc];
        [Bbtn addTarget:vc action:@selector(click) forControlEvents:UIControlEventTouchUpInside];
        if (self.hasback) {
...
- (UIButton *)playOrPauseBtn{
    if (_playOrPauseBtn == nil) {
        ...
        [_playOrPauseBtn setImage:[UIImage imageNamed:@"live_icon_play"] forState:UIControlStateNormal];
...
- (UIButton *)fullOrSmallBtn{
    if (_fullOrSmallBtn == nil) {
        ...
        [_fullOrSmallBtn setImage:[UIImage imageNamed:@"icon_fullscreen"] forState:UIControlStateNormal];
        isFull = NO;
...
- (void)layoutSubviews{
    ...
    self.bottomMenuView.alpha = 0.9;
    self.playOrPauseBtn.frame = CGRectMake(self.bottomMenuView.left+5, 5, 30, 30);
    if (isFull) {
        self.nextPlayerBtn.frame = CGRectMake(self.playOrPauseBtn.right+5, 5, 30, 30);
        self.bottomMenuView.frame = CGRectMake(0, self.height-40, self.width, 40);
        ...
        self.nextPlayerBtn.frame = CGRectMake(self.playOrPauseBtn.right+10, 5, 0, 0);
    ...
    self.loadProgressView.trackTintColor = KWhiteColor;
    self.loadProgressView.progressTintColor = RGB(69, 163,229);
