...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSString *tempMV =  [document stringByAppendingPathComponent:@"xjPlayer"];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (self.taskArr.count < 2) {
        ...
        //这里自己写需要保存数据的路径
        NSString *document = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject;
        NSString *movePath =  [document stringByAppendingPathComponent:[NSString stringWithFormat:@"xjPlayer/%@",self.savePath]];
