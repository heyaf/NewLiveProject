    ...
    dispatch_source_set_event_handler(_timer, ^{
        if (timeout <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSMutableAttributedString *attrString = [self setMutableAttributedStringWithString:string color:RGB(69, 163, 229) range:NSMakeRange(0, string.length)];
                [self setAttributedTitle:attrString forState:UIControlStateNormal];
                
                self.layer.borderColor = RGB(69, 163, 229).CGColor;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSMutableAttributedString *attrString = [self setMutableAttributedStringWithString:string color:kColor(grayColor) range:NSMakeRange(0, string.length) subStrColor:RGB(69, 163, 229) subStrRange:range];
                [self setAttributedTitle:attrString forState:UIControlStateNormal];
