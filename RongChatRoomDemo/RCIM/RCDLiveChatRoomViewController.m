...
//全屏按钮
@property (nonatomic,strong) UIButton *fullScrBtn;
         ...
         success:^{
             dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //                  self.livePlayingManager = [[LELivePlaying alloc] initPlaying:@"201604183000000z4"];
                 ...
                 joinChatroomMessage.message = [NSString stringWithFormat: @"%@进入房间",[RCDLive sharedRCDLive].currentUserInfo.name];
...
- (void)leftBarButtonItemPressed:(id)sender {
    if (_isScreenVertical) {
        [self changeCrossOrVerticalscreenByButton:YES];
    }else{
...
- (void)initializedSubViews {
    ...
    if(self.contentView == nil){
        CGRect contentViewFrame = CGRectMake(0, KScreenW/16*9+90, KScreenW,KScreenH-(KScreenW/16*9+90));
...
- (void)initializedLiveSubViews {
    ...
    UIImageView *imagev = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, KScreenW,KScreenW* 9 / 16)];
    imagev.image = [UIImage imageNamed:@"Playnostart"];
    [_liveView addSubview:imagev];
     _liveView = self.livePlayingManager.currentLiveView;
    _liveView.frame = CGRectMake(0, 0, KScreenW, KScreenW* 9 / 16);
    [self.view addSubview:_liveView];
    [self.view sendSubviewToBack:_liveView];
    [self.view sendSubviewToBack:_headerView];
    
    _fullScrBtn = [[UIButton alloc] initWithFrame:CGRectMake(KScreenW-40, KScreenW* 9 / 16-40, 30, 30)];
    [_fullScrBtn setImage:[UIImage imageNamed:@"icon_fullscreen"] forState:UIControlStateNormal];
    [_fullScrBtn addTarget:self action:@selector(fullScreenBtn) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_fullScrBtn];
...
-(void)fullScreenBtn{
    ...
    if (!_isScreenVertical) {
        ...
        [self changeCrossOrVerticalscreenByButton:NO];
    } else {
        [self changeCrossOrVerticalscreenByButton:YES];
...
//
//    NSLog(@".................切换按钮，，，，，，，，");
//    self.livePlayingManager.currentLiveView.frame = CGRectMake(0, 0, KScreenW, KScreenW* 9 / 16);
...
- (void)onInputBarControlContentSizeChanged:(CGRect)frame withAnimationDuration:(CGFloat)duration andAnimationCurve:(UIViewAnimationCurve)curve{
    ...
    collectionViewRect.origin.y = (KScreenW/16*9+90) - frame.size.height  +50;
    
    collectionViewRect.size.height = KScreenH-(KScreenW/16*9+90);
...
-(void)changeCrossOrVerticalscreenByButton:(BOOL)isVertical{
    ...
    if (!isVertical) {
        ...
        _fullScrBtn.frame = CGRectMake(10, KScreenH-40, 30, 30);
//        frame = CGRectMake(10, 100,30, 30);
        _backBtn.frame = CGRectMake(KScreenW-40, 10, 30, 30);
        ...
        _fullScrBtn.frame = CGRectMake(KScreenW -40, KScreenW* 9 / 16-40, 30, 30);
        ...
        [_backBtn setTransform:CGAffineTransformMakeRotation(0*M_PI/180)];
