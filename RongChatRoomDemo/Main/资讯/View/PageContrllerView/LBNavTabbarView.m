...
- (instancetype)initWithFrame:(CGRect)frame andVCArr:(NSArray<UIViewController *> *)vcArr andTitleArr:(NSArray <NSString *>*)titleArr lineHeight:(CGFloat)lineHeight currentVC:(UIViewController *)currentVC
{
    if (self = [super init]) {
        self.frame = frame;
        self.selectedTColor = RGB(17, 157, 255);
        self.downLineColor = RGB(17, 157, 255);
...
- (UIScrollView *)itemScroll
{
    if (_itemScroll == nil) {
        ...
        _itemScroll.alwaysBounceVertical = NO;
        ...
        for (int i = 0; i < self.titleArr.count; i++) {
            ...
            if (self.titleArr.count<6) {
                W =(KScreenW-PADDING*2)/(self.titleArr.count+1);
            }else{
                W = size.width;
            }
            btn.frame = CGRectMake(originX, 0, W, self.frame.size.height - self.lineHeight);
            if (self.titleArr.count ==2) {
                btn.frame = CGRectMake(SCREEN_WIDTH/2*i, 0, SCREEN_WIDTH/2, self.frame.size.height - self.lineHeight);
            }
        ...
        _itemScroll.contentSize = CGSizeMake(btnOffset+PADDING, 0);
    }
    return _itemScroll;
}
//初始化操作
...
{
    if (_showScroll == nil) {
        _showScroll = [[UIScrollView alloc]initWithFrame:CGRectMake(0, self.frame.origin.y + self.itemScroll.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT-48- self.frame.origin.y + self.itemScroll.frame.size.height)];
        _showScroll.contentSize = CGSizeMake(SCREEN_WIDTH * self.titleArr.count , SCREEN_HEIGHT -48- self.frame.origin.y + self.itemScroll.frame.size.height);
        ...
        _showScroll.alwaysBounceVertical = NO;
        ...
        [self.currentVC.view addSubview:_showScroll];
    }
    return _showScroll;
}
